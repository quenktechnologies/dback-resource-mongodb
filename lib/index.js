...
const defaultSearchParams = { page: 1, limit: 5000, query: {}, sort: {}, fields: {} };
...
class BaseResource {
    ...
    create(r) {
        ...
        return api_1.doAction(function* () {
            ...
            yield response_1.created({ data: { id } });
            yield that.after(r);
    ...
    search(r) {
        ...
        return api_1.doAction(function* () {
            ...
            let params = r.prs.getOrElse(exports.KEY_SEARCH_PARAMS, defaultSearchParams);
            let db = yield pool_1.checkout(that.conn);
            let model = that.getModel(db);
